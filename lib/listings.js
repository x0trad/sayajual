...
export async function createListing({ sourceUrl, title, items }) {
  await ensureSchema();
  ...
  try {
    ...
    await client.query(
      `
      INSERT INTO listings (id, slug, title, source_url, created_at)
      VALUES ($1, $2, $3, $4, $5)
      `,
      [listing.id, listing.slug, listing.title, listing.sourceUrl, listing.createdAt]
    );
